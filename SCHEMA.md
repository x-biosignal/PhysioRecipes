...
synced to the public repo until the paper is out. Such cases live in a private,
un-synced location outside the `Physio*` glob, so the publisher never sees them. Enforcement: **no case under `inst/cases/` may set
