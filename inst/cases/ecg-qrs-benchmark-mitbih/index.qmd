...
categories: [ecg, detection, physionet, benchmark, verified]
...
> R-peak detector reaches clinical-grade accuracy (pooled **Se 0.966 · PPV 0.976 ·
> F1 0.971**) across **8 records** from clean sinus to the noisiest. **What
...
ecosystem's `pan_tompkins` R-peak detector reaches pooled **Se 0.966**, **PPV
0.976**, **F1 0.971** — clinical-grade — with per-record F1 from **1.0000** on
...
![Per-record F1 against the cardiologist ground truth, clean records to the noisiest (203); dashed = 0.95 acceptance bar, dotted = pooled F1 0.971.](artifacts/figure.png)
...
|---|---|---|---|---|---|---|---|---|
| 115 | clean | 1953 | 1953 | 0 | 0 | 1.0000 | 1.0000 | **1.0000** |
| 100 | clean | 2273 | 2271 | 1 | 2 | 0.9991 | 0.9996 | 0.9993 |
| 234 | clean | 2753 | 2749 | 0 | 4 | 0.9985 | 1.0000 | 0.9993 |
| 117 | clean | 1535 | 1534 | 2 | 1 | 0.9993 | 0.9987 | 0.9990 |
| 103 | clean | 2084 | 2078 | 0 | 6 | 0.9971 | 1.0000 | 0.9986 |
| 214 | moderate | 2262 | 2235 | 20 | 27 | 0.9881 | 0.9911 | 0.9896 |
| 105 | moderate | 2572 | 2547 | 63 | 25 | 0.9903 | 0.9759 | 0.9830 |
| **203** | **noisy** | 2980 | 2415 | 353 | 565 | 0.8104 | 0.8725 | **0.8403** |
| **GROSS** | pooled | 18412 | 17782 | 439 | 630 | **0.9658** | **0.9759** | **0.9708** |

- On the **clean** records the detector is essentially exact (F1 ≥ 0.999).
...
- **Pooled Se 0.966, PPV 0.976, F1 0.971** all clear the stated 0.95
  clinical-grade bar.
...
frozen pre-registration (`prereg.json`, sha256 `9d268f9c3498...`), a run
manifest with content-addressed output hashes, and a verification report
grounding all 3 claims (`verification_report.json`). Replay is
byte-identical: re-executing the frozen op-DAG reproduces the terminal
artifact hash (`7ec3357fb8ed480c`) exactly.

## Erratum (PhysioRecipes 0.7.2) {#erratum}

The per-record and pooled scores on this page were corrected in release 0.7.2
after a data-correctness fix in the reader.

`PhysioIO::readWFDB()` mis-defaulted the signal baseline on headers that state
the gain without a `(baseline)` annex — the style of the original MIT-BIH
headers — taking the gain as the baseline instead of the WFDB default (the ADC
zero). Every physical value therefore carried a **+4.12 mV** offset. Fixed in
PhysioIO 0.2.7; the corrected reader agrees with the reference implementation
`wfdb-python` **bit-for-bit** (max |difference| = 0) on these records.

The offset mattered most where it pushed the detector's amplitude logic off:
on **record 100** it cost 139 beats (FN 141 → **2**, F1 0.9678 → **0.9993**),
moving that record from the "moderate" to the "clean" tier, with smaller gains
on records 115, 103 and 105. Pooled scores rose from Se 0.9578 / PPV 0.9755 /
F1 0.9666 to **Se 0.9658 / PPV 0.9759 / F1 0.9708**.

The ground truth itself is unchanged: `artifacts/ref_beats.json` holds beat
annotations extracted from each `.atr` with WFDB 4.3.1 and never passed through
the defective path. The case's finding — clinical-grade pooled accuracy with an
honest clean-to-noisy gradient, and record 203 still the floor at F1 0.84 — is
unchanged in substance; only the numbers are now those of correctly scaled
signals.
