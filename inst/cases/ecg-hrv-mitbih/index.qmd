...
categories: [ecg, hrv, physionet, cross-tool, verified]
...
mean HR = **75.51 bpm**, mean RR = **794.63 ms**, detected over the **full
30-minute record 100** (MLII). The HRV table above is a different analysis of
the same record — its **first five minutes**, using the cardiologists' own beat
annotations rather than detected peaks — so the two heart-rate figures are
close but not identical by construction.
...
The resting-HR figures above were corrected in release 0.7.2. Two independent
defects were found and fixed; both are recorded here rather than quietly
overwritten.

**1. The run's input was a synthetic test fixture, not record 100.** Until
0.7.2 this case's pipeline read `PhysioIO/inst/extdata/100` — a *synthetic*
format-212 record bundled inside PhysioIO purely to exercise the WFDB codec
(so labelled in the commit that introduced it), whose SHA-256 the shipped
`bundle/proposal.json` recorded. The case nevertheless described its input as
MIT-BIH record 100. The pipeline now reads the tracked PhysioNet record 100
itself — the same raw input as the sibling MIT-BIH cases — and the reported
values (mean HR 72.03 → **75.51 bpm**, mean RR 832.99 → **794.63 ms**) are
those of the real 30-minute recording. A sweep of all 114 cases found this to
be the only case whose input was a package test fixture.

**2. `readWFDB()` mis-defaulted the baseline on bare-gain headers.** MIT-BIH
headers state the gain without a `(baseline)` annex, and the reader took the
gain as the baseline instead of the WFDB default (the ADC zero), offsetting
every physical value by +4.12 mV. Fixed in PhysioIO 0.2.7; the corrected reader
now agrees with the reference implementation `wfdb-python` **bit-for-bit**
(max |difference| = 0) on this record.

Neither defect touched the cross-tool HRV table or the detection table on this
page: both derive from `PhysioECG`'s MIT-BIH reference fixture, which was built
with `wfdb` 4.3.1 from the real annotated record and is therefore unaffected.
The case's conclusion is unchanged — record 100 shows a normal resting heart
rate — and is now supported by the recording it names.

## Replay bundle (verified tier)

This case carries a substrate replay bundle under [`bundle/`](bundle/): the
frozen pre-registration (`prereg.json`, sha256 `7367de3afa79...`), a run
manifest with content-addressed output hashes, and a verification report
grounding all 2 claims (`verification_report.json`). Replay is
byte-identical: re-executing the frozen op-DAG reproduces the terminal
artifact hash (`af49efe151cc44b8`) exactly.
