...
This case carries no replay bundle. Under the decomposition shipped in
PhysioHDEMG 0.2.0 — the version the replay harness runs — reference **MU5** sits
at the separability floor (lag-aware RoA ≈ 0), whereas the published value above
(`mu5_roa`) is **0.950**; the difference is far beyond the gate tolerance
`max(0.02, |v|·1e-3)`. This is distinct from the case's own documented finding
that MU3 (RoA 0.017) is not recovered. The published values are kept as authored
and traced to `artifacts/hdemg_roa.csv`; the case stays validated-tier with this
boundary stated rather than re-tuned.
