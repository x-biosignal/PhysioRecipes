...
# the finding (re-authored in 0.7.2): heavy-tailed, near-symmetric RR -> robust < classical
check("RR distribution is heavy-tailed (excess kurtosis > 3)", sm["rr_excess_kurtosis"] > 3)
check("RR distribution is near-symmetric (|skewness| < 0.5) -- the non-Gaussianity is in the tails",
      abs(sm["rr_skewness"]) < 0.5)
...
check("HONEST: the distribution-shape diagnostic finding (heavy-tailed RR -> robust < classical; SDNN inflated)",
      "distribution-shape diagnostic" in _note and "heavy-tailed" in _note and "sdnn is inflated" in _note)
