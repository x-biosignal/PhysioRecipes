...
**Erratum release.** A data-correctness defect in `PhysioIO::readWFDB()` (fixed
in PhysioIO 0.2.7) had offset every physical value read from MIT-BIH- and
Fantasia-style WFDB headers, and one case additionally ran on a synthetic test
fixture while claiming real MIT-BIH provenance. Fourteen cases were audited and
re-certified against corrected data; the corrections, including the one finding
that did not survive, are documented in a per-case `## Erratum` section rather
than silently overwritten.

## The reader defect

A WFDB signal line whose gain field carries no `(baseline)` annex — the style
of the original MIT-BIH and Fantasia headers — was read with `baseline = gain`
instead of the spec default `baseline = ADC zero`, offsetting every sample by
`(adczero - gain)/gain`: **+4.12 mV on MIT-BIH**, **exactly −1 physical unit on
Fantasia**. Headers carrying an explicit `(baseline)` (BIDMC, GRABMyo, Non-EEG)
were unaffected, as were all non-WFDB modalities. The corrected reader now
reproduces the reference implementation **`wfdb-python` bit-for-bit
(max |difference| = 0)** on both datasets — the validation this release rests on.

## What changed, by case

* **`ecg-hrv-mitbih`** — two defects. Its pipeline read
  `PhysioIO/inst/extdata/100`, a *synthetic* codec-test fixture (so labelled in
  the commit that created it, and confirmed by the SHA-256 recorded in the
  shipped bundle), while the case described its input as MIT-BIH record 100. It
  now reads the tracked PhysioNet record itself: mean HR 72.03 → **75.51 bpm**,
  mean RR 832.99 → **794.63 ms**. A sweep of all 114 cases confirmed this was
  the only case whose input was a package test fixture. The page's cross-tool
  HRV and detection tables were unaffected (they derive from PhysioECG's
  `wfdb` 4.3.1-built reference fixture).
* **`ecg-qrs-benchmark-mitbih`** — the offset cost the detector 139 beats on
  record 100 (FN 141 → **2**, F1 0.9678 → **0.9993**, moving it from the
  "moderate" to the "clean" tier), with smaller gains on 115/103/105. Pooled
  Se/PPV/F1 0.9578/0.9755/0.9666 → **0.9658/0.9759/0.9708**. The `.atr` ground
  truth never passed through the defective path and is unchanged.
* **`iqr-hrv-fantasia`** — **the one finding that did not survive.** The case
  reported a *right-skewed* RR distribution (skewness 1.01); that skew was an
  artefact of a beat the offset made the detector miss. Corrected, the
  distribution is **near-symmetric but heavy-tailed** (skewness 0.07, excess
  kurtosis 7.17). The case has been re-authored accordingly — including its
  audit invariant, from `skewness > 0.5` to `excess kurtosis > 3 and
  |skewness| < 0.5`, so the automated check enforces the finding actually
  claimed. Its core result is unchanged and sharper: `interquartileRange` still
  reproduces `scipy.stats.iqr` and base R `IQR` bit-for-bit, and the correction
  moved SDNN by 4.4 ms while leaving the **IQR at exactly 76 ms** — the case's
  own thesis, demonstrated by a real perturbation.
* **Nine further Fantasia HRV cases** (`poincare-hrv`, `hra`, `csi-cvi`,
  `hti-tinn`, `hrv-time-neurokit`, `trimmed-mean-hrv`, `hrv-dfa-aging`,
  `hrv-lomb`, and the `ecg-hrv-aging` cohort) were re-run on RR roots
  re-extracted with the corrected reader (+24 beats across the 10 subjects;
  the f1y01 five-minute window 391 → **392** intervals). Every finding-level
  invariant survived; the elderly subjects reproduced bit-for-bit throughout, an
  internal consistency check. Two honest consequences are now reported rather
  than smoothed: `csi-cvi`'s NeuroKit2 agreement is no longer exactly 0 but at
  the floating-point floor (~1e-13), and NeuroKit2's TINN fit is **degenerate
  (0 ms)** on the corrected series, so `hti-tinn` reports a 164-vs-0 ms gap.
* **`ecg-hr-spectrum-mitbih`** — record 117, which carries the claims, is
  unchanged to 10 decimals. Its secondary contrast records did move (115: mean
  RR 924.3 → **924.7**; 103: 868.6 → **868.8**) and were re-derived.
* **`ecg-pvc-mitbih`** — unchanged: its reference-beat protocol scores the
  cardiologists' own beat locations and is insensitive to the offset.

## Provenance and tooling

* Every re-certified case ships a **new replay bundle** produced by the
  corrected pipeline; all 114 cases pass the tier gate and their per-case
  audits (0 failures), and the tier split is unchanged at **109 verified /
  5 validated**.
* The corrected numbers, artifact tables and figures are now produced by
  **committed generators** (`agent/make_fantasia_rr_roots.R`,
  `agent/make_<case>_artifacts.R`, `agent/make_<case>_figure.R`). Their absence
  is what made this correction laborious: the original artifacts had been
  written ad hoc. Every figure that displayed a changed number was re-rendered
  from the corrected artifacts.

# PhysioRecipes 0.7.1

**Metadata and documentation release — no case value, pipeline, bundle or tier changes.**

* Dataset license harmonised: the four GRABMyo cases' `artifacts/data_sources.json`
  now all record `CC-BY 4.0` (PhysioNet grabmyo 1.0.2 and 1.1.0 both state the
  Creative Commons Attribution 4.0 International Public License; one record had said
  ODC-By v1.0 and one "PhysioNet Open Access"), and `emg-spacetime-synergy-grabmyo`
  now carries the version DOI 10.13026/89dm-f662 stated on the PhysioNet page (its
  former 10.13026/rd4p-4w49 is not stated there).
* Dataset DOIs added where the source page states one: Fantasia 10.13026/C2RG61
  (10 cases), BIDMC 10.13026/C2208R (6), Sleep-EDF 10.13026/C2X676 (4), TESA example
  data 10.4225/03/5719CEBC59438 (figshare record 3188800), Murphy et al. 2018
  10.1371/journal.pbio.2002811 (the article the musculoskeletal network is digitised
  from), VerbAgg 10.32614/CRAN.package.lme4 (the CRAN package that ships the data).
  No DOI is stated for the MNE fNIRS tutorial recording or the openhdemg sample
  recording; those two records are unchanged.
* The five validated-tier cases now state on their own page (`index.qmd`, section
  "Why this case is validated, not verified") why they carry no replay bundle, with
  the recomputed value where there is one: `gait-promp-fukuchi` (via-point pull
  25.62° recomputed vs 25.8° published; `promCondition` version drift),
  `hdemg-mu-openhdemg` (MU5 at the separability floor, RoA ≈ 0, under PhysioHDEMG
  0.2.0 vs 0.950 published), `emg-spacetime-synergy-grabmyo` (LOO gesture accuracy
  8/14 = 0.571 recomputed vs 9/14 = 0.643 published), `eeg-berger-bids-physiopy`
  (Python physiopy/BIDS pipeline, no R op-DAG spec), `gait-multimodal-fusion-fukuchi`
  (unpublished heel-strike segmentation recipe; faithful reconstruction does not
  reproduce the published values).
* Gates unchanged and green: `tools/validate_case.py` + per-case `audit.py` pass for
  all 114 cases (109 verified · 5 validated).

# PhysioRecipes 0.7.0

**Verified-tier promotion campaign: 6 → 109 of 114 cases now ship a replay bundle.**

* 103 cases promoted validated → verified. Each promoted case now carries a substrate
  replay bundle under `inst/cases/<id>/bundle/`: the frozen pre-registration
  (`prereg.json`, bound to `case.json` by its SHA-256), a run manifest with
  content-addressed output hashes, a claim-by-claim `verification_report.json`
  grounding **every** case claim, the terminal and external-reference comparison
  tables, and a provenance summary. Replay is byte-identical: re-executing the
  frozen op-DAG reproduces the terminal artifact hash exactly (stochastic operators
  under their recorded seed).
* Multi-record / cohort headline claims (pooled QRS and PVC beat-detection scores
  over 8 and 9 MIT-BIH records, 20-subject Fantasia young-vs-elderly comparisons,
  the 53-record BIDMC PPG-vs-ECG agreement analysis, the 15-subject EMG-fatigue
  cohort, two-night Sleep-EDF spectra, 64-channel EEG topography aggregates,
  split-half synergy reliability, per-activity bilateral arm use, and the
  MBLL-vs-MNE cross-language check) are grounded by an extended reference layer
  that **recomputes each aggregate independently from the raw public recordings**
  — never from the case's own artifacts.
* 5 cases remain validated-tier, each for a documented, honest reason stated in the
  case itself: `eeg-berger-bids-physiopy` (Python/BIDS pipeline),
  `gait-promp-fukuchi` (via-point conditioning version drift),
  `hdemg-mu-openhdemg` (MU5 at the separability floor),
  `gait-multimodal-fusion-fukuchi` (unpublished segmentation recipe),
  `emg-spacetime-synergy-grabmyo` (LOO gesture accuracy from a custom
  preprocessing chain whose raw multi-trial records are not tracked; faithful
  reconstruction gives 8/14 vs the published 9/14).
* Gallery: tier badges now reflect the split (109 verified · 5 validated); every
  promoted case page documents its replay bundle; the landing glance table and the
  generated coverage map are rebuilt from the corpus.
* Data hygiene: `artifacts/data_sources.json` is now present in **all 114 cases**
  (licenses and DOIs for the 8 backfilled records verified against the source
  repositories: MIT-BIH/eegmmidb/Non-EEG = ODC-By v1.0, GRABMyo = CC BY 4.0, with
  registered PhysioNet DOIs); `cohort-longitudinal-parkinsons-updrs` gained its
  missing gallery date.
* Integrity gates unchanged and green: `tools/validate_case.py` (tier-aware
  invariants, including `prereg_hash == sha256(bundle/prereg.json)` and
  `n_grounded == n_claims` for verified cases) + per-case `audit.py` pass for all
  114 cases.

# PhysioRecipes 0.6.85

* New validated case `gait-grf-variance-levene-gaitrec` — pathology flattens AND constrains the GRF.
  Authoring `PhysioAnalysis::leveneTest` (robust Brown-Forsythe) and reusing the certified
  grfLandmarks-derived dynamic range, it tests whether the five GaitRec gait classes differ in variance.
  They do (Levene F(4,1839)=3.42, p=0.0086, healthy controls most variable at SD 0.128 vs pathology ~0.11)
  — though the normality-sensitive Bartlett test misses it (p=0.079); the two disagree, showing why the
  robust Levene is preferred. Pathology both flattens the dynamic range (lower mean) and constrains it
  (lower variance). `leveneTest` reproduces `scipy.stats.levene` and `car::leveneTest` to machine precision.

# PhysioRecipes 0.6.84

* New validated case `sway-retest-icc-bds` — a single quiet-standing trial is reliable. Authoring
  `PhysioAnalysis::intraclassCorrelation` and reusing the certified `PhysioMoCap::swayMetrics`, it
  quantifies the test-retest reliability of COP path length across 3 repeated eyes-open-firm trials in 163
  BDS subjects. A single trial has good reliability (ICC(2,1)=0.825) and the 3-trial average is excellent
  (ICC(2,k)=0.934) — single-trial posturography is trustworthy, averaging improves it (Spearman-Brown).
  `intraclassCorrelation` reproduces `psych::ICC(lmer=FALSE)` and `pingouin.intraclass_corr` to machine
  precision. Extends the reliability/psychometrics thread (with `cronbachAlpha`).

# PhysioRecipes 0.6.83

* New validated case `fes-reliability-cronbach-bds` — the Falls Efficacy Scale is internally consistent.
  Authoring `PhysioAnalysis::cronbachAlpha` (a psychometrics op), it certifies the internal-consistency
  reliability of the 7 FES items in the BDS cohort (n=163): Cronbach's α = 0.823 (mean inter-item r = 0.41),
  good internal consistency — the items measure a coherent single construct, justifying the summed FES score
  the sway-cognition cases relied on. `cronbachAlpha` reproduces `psych::alpha` bit-for-bit and
  `pingouin.cronbach_alpha` to machine precision. Broadens the gallery into psychometrics.

# PhysioRecipes 0.6.82

* New validated case `sway-cognition-mlr-bds` — the full multivariable model. Authoring
  `PhysioAnalysis::multipleRegression` and reusing the certified swayMetrics-derived sway, it fits
  eyes-closed-foam sway ~ age + Trail Making Test A, the fuller model the partial-correlation sibling
  called for. Age is the dominant predictor (β=1.59 cm/year, p=2e-8) while TMT-A's independent contribution
  is not significant (β=0.12, p=0.55); the model explains ~28% of sway variance (R²=0.276, F(2,155)=29.5).
  Cognitive speed adds nothing beyond age. `multipleRegression` reproduces base R `lm` bit-for-bit and
  `statsmodels` OLS to machine precision. Resolves the open question of `sway-cognition-partial-bds`.

# PhysioRecipes 0.6.81

* New validated case `sway-4cond-nemenyi-bds` — vision matters far more on foam. Authoring
  `PhysioAnalysis::nemenyiTest` (Nemenyi post-hoc after Friedman) and reusing the certified
  swayMetrics-derived per-condition sway, it runs the post-hoc the Friedman sibling deferred. All six
  vision × surface condition pairs differ, but the vision effect depends on the surface: eyes-open vs
  eyes-closed is only borderline on firm ground (p=0.044) yet strong on foam (p=3.1e-8) — when
  proprioception is degraded, balance reweights toward vision. `nemenyiTest` reproduces
  `PMCMRplus::frdAllPairsNemenyiTest` bit-for-bit and `scikit_posthocs.posthoc_nemenyi_friedman` to machine
  precision. Resolves the open question of `sway-4cond-friedman-bds`, completing the post-hoc set
  (Tukey/Dunn/Nemenyi).

# PhysioRecipes 0.6.80

* New validated case `gait-grf-pushoff-dunn-gaitrec` — push-off grades gait pathology. Authoring
  `PhysioAnalysis::dunnTest` (Dunn's post-hoc after Kruskal-Wallis) and reusing the certified
  grfLandmarks-derived push-off peak, it runs the post-hoc the Kruskal-Wallis sibling deferred. Healthy
  controls differ from all four pathology classes (4/4), and -- unlike the dynamic range, where Tukey
  found 0/6 pathology-pathology pairs -- the push-off peak separates 4/6 pathology-pathology pairs
  (calcaneus distinct from ankle/knee/hip, hip from ankle): the dynamic range flags impairment, the
  push-off peak grades it. `dunnTest` reproduces `PMCMRplus::kwAllPairsDunnTest` and
  `scikit_posthocs.posthoc_dunn` to machine precision. Resolves the open question of
  `gait-grf-pushoff-kruskal-gaitrec`.

# PhysioRecipes 0.6.79

* New validated case `gait-grf-tukey-pathology-gaitrec` — which gait classes actually differ. Authoring
  `PhysioAnalysis::tukeyHSD` (all pairwise comparisons after one-way ANOVA) and reusing the certified
  grfLandmarks-derived dynamic range, it runs the post-hoc the ANOVA sibling deferred. Healthy controls
  differ significantly from all four pathology classes (every adjusted p < 2e-11) while none of the six
  pathology-pathology pairs differ (smallest p = 0.61): the GRF dynamic range separates healthy from
  pathological gait but not the pathologies from one another (the push-off case shows a different landmark
  grades them). `tukeyHSD` reproduces base R `TukeyHSD` and `scipy.stats.tukey_hsd` to machine precision.
  Resolves the open question of `gait-grf-anova-pathology-gaitrec`.

# PhysioRecipes 0.6.78

* New validated case `sway-cognition-partial-bds` — the cognitive-motor link is mostly age. Authoring
  `PhysioAnalysis::partialCorrelation` and reusing the certified `PhysioMoCap::swayMetrics`, it adjusts
  the sibling case's sway-cognition association for age. The rank partial correlation attenuates from a
  marginal Spearman rho of 0.532 to 0.201 (p=0.012, still significant but much weaker), and the linear
  (Pearson) partial correlation vanishes (0.048, p=0.55): most of the marginal link was age, though a
  modest rank association survives. `partialCorrelation` reproduces `ppcor::pcor.test` and an independent
  closed-form recompute to machine precision. Honestly resolves the open question of
  `sway-cognition-kendall-bds`.

# PhysioRecipes 0.6.77

* New validated case `sway-cognition-kendall-bds` — the cognitive-motor link. Authoring
  `PhysioAnalysis::kendallTau` (Kendall's tau-b, completing the rank-correlation trio) and reusing the
  certified `PhysioMoCap::swayMetrics`, it correlates eyes-closed-foam COP path length with Trail Making
  Test A time across 158 BDS subjects. Slower processing speed tracks more sway: tau-b = 0.368, z = 6.85,
  p ~ 7.2e-12; on the same data Pearson r = 0.335 < Spearman rho = 0.532 (monotonic > linear, so a rank
  correlation is preferred). `kendallTau` reproduces `scipy.stats.kendalltau` and base R
  `cor.test(method="kendall")` bit-for-bit. Honest limits: marginal/unadjusted (age a shared correlate),
  cross-sectional.

# PhysioRecipes 0.6.76

* New validated case `gait-pushoff-age-regression-gaitrec` — the healthy-aging propulsion decline.
  Authoring `PhysioAnalysis::linearRegression` (simple OLS) and reusing the certified
  `PhysioMoCap::grfLandmarks`, it regresses the vertical-GRF push-off peak on age in GaitRec healthy
  controls (n=208, ages 15-78). The push-off peak declines at slope −0.00120 BW/year (−0.012 BW/decade;
  intercept 1.144 BW), t(206)=−4.61, p~7.0e-6, R²=0.094 — a modest but highly significant decline (age
  explains ~9% of the between-subject variance). `linearRegression` reproduces `scipy.stats.linregress`
  bit-for-bit and base R `lm` to machine precision. Honest limits: young-skewed ages, cross-sectional
  (not longitudinal).

# PhysioRecipes 0.6.75

* New validated case `gait-grf-pushoff-kruskal-gaitrec` — the push-off peak discriminates gait pathology.
  Authoring `PhysioAnalysis::kruskalTest` (the non-parametric distribution-free counterpart of
  `oneWayAnova`) and reusing the certified `PhysioMoCap::grfLandmarks`, it compares the vertical-GRF
  push-off peak (late-stance propulsion) across five GaitRec gait classes. Because the push-off peak is
  right-skewed, Kruskal-Wallis is the appropriate omnibus: the classes differ (H(4)=274.5, p~3.5e-58)
  with healthy controls highest (mean 1.102 BW) and calcaneus pathology lowest (1.008 BW), ankle/knee/hip
  in between — unlike the dynamic range (ANOVA sibling), which clustered the pathology classes, the
  push-off peak separates them, the calcaneus propulsion deficit being most severe. `kruskalTest`
  reproduces base R `kruskal.test` bit-for-bit and `scipy.stats.kruskal` to machine precision.

# PhysioRecipes 0.6.74

* New validated case `sway-4cond-friedman-bds` — the posturographic sensory hierarchy, tested correctly
  for repeated measures. Authoring `PhysioAnalysis::friedmanTest` (the non-parametric within-subject
  counterpart of the certified `oneWayAnova`) and reusing the certified `PhysioMoCap::swayMetrics`, it
  compares COP path length across the four BDS vision × surface quiet-standing conditions on the 158
  subjects measured under all four. The conditions differ overwhelmingly (Friedman Q(3)=420.1,
  p~9.7e-91) in a monotone mean-rank order Open-Firm (1.31) < Closed-Firm (1.69) < Open-Foam (3.08) <
  Closed-Foam (3.92): each sensory challenge adds sway, and softening the surface (proprioception)
  dominates closing the eyes (vision). `friedmanTest` reproduces base R `friedman.test` bit-for-bit and
  `scipy.stats.friedmanchisquare` to machine precision. Closes the sibling `romberg-vision-sway-bds`
  case's 2×2 open question with the correct repeated-measures test.

# PhysioRecipes 0.6.73

* New validated case `gait-grf-anova-pathology-gaitrec` — flattened GRF as a shared signature of
  impaired gait, certified with an omnibus one-way ANOVA. Authoring `PhysioAnalysis::oneWayAnova`
  (the many-group generalisation of the certified `twoSampleTTest`) and reusing the certified
  `PhysioMoCap::grfLandmarks`, it compares the vertical-GRF dynamic range (loading peak minus
  mid-stance trough) across five GaitRec gait classes — healthy control (n=208), ankle (n=483),
  knee (n=499), hip (n=364), calcaneus (n=290). The dynamic range differs across classes
  (F(4,1839)=73.4, p~8.9e-58), driven by healthy controls (mean 0.365 BW) having ~1.65x the range of
  the tightly clustered pathology classes (0.215–0.227 BW): GRF flattening is not knee-specific.
  `oneWayAnova` reproduces base R `oneway.test(var.equal=TRUE)` bit-for-bit and `scipy.stats.f_oneway`
  to machine precision. Generalises the sibling `gait-grf-dynrange-pathology-gaitrec` case, closing its
  open question.

# PhysioRecipes 0.6.72

* New validated case `gait-grf-dynrange-pathology-gaitrec` — the flattened ground-reaction force of
  impaired gait, certified as a group difference. Reusing the certified `PhysioAnalysis::twoSampleTTest`
  and `PhysioMoCap::grfLandmarks` (a new finding, not new ops), it compares the vertical-GRF dynamic range
  (loading peak minus mid-stance trough) of healthy controls (n=208) vs knee-pathology patients (n=499)
  in the GaitRec dataset. Controls have a much larger dynamic range (mean 0.367 vs 0.196 BW, ratio ~1.87,
  Welch t = 17.3 on 337 df, p ~ 2.8e-48) — pathological gait flattens the GRF double-hump. The Welch t
  reproduces **both** `scipy.stats.ttest_ind(equal_var = FALSE)` **and** base R `t.test` **bit-for-bit**,
  turning the sibling `gait-grf-gaitrec` case's descriptive landmark means into a certified inferential
  result.

# PhysioRecipes 0.6.71

* New validated case `romberg-vision-sway-bds` (the 100th validated case) — the visual contribution to
  balance. Reusing the certified `PhysioAnalysis::pairedTTest` (a new finding, not a new op), it compares
  eyes-closed vs eyes-open foam-surface COP path length (from the certified `swayMetrics`) across 50
  subjects of the Santos & Duarte (2016) balance dataset. Eyes-closed sway (mean 217 cm) exceeds eyes-open
  (mean 167 cm), a ratio of ~1.30, highly significant (t = 7.57 on 49 df, p ~ 8.8e-10), and eyes-closed is
  greater in 43 of 50 subjects — the Romberg / vision effect. The paired t reproduces **both**
  `scipy.stats.ttest_rel` **and** base R `t.test(paired = TRUE)` **bit-for-bit**. Together with the
  foam-vs-firm (proprioception) and age effects, this completes the posturographic sensory triad — the
  three inputs to standing balance.

# PhysioRecipes 0.6.70

* New validated case `age-group-sway-bds` — a **group-comparison cohort** case introducing
  `PhysioAnalysis::twoSampleTTest` (Welch). Splitting the Santos & Duarte (2016) cohort into older (n=11)
  and younger (n=39) subjects, foam-surface COP path length (from the certified `swayMetrics`) is markedly
  greater in the older group — mean 227 vs 151 cm, ratio ~1.5, Welch t = 3.86 on 14.5 df, p = 1.6e-3.
  The two-sample test reproduces **both** `scipy.stats.ttest_ind(equal_var = FALSE)` **and** base R
  `t.test` **bit-for-bit**. This is the age-related balance decline as a group comparison, complementing
  the continuous age-sway correlation (sibling case), on a chained certification (inputs from the
  separately-certified swayMetrics).

# PhysioRecipes 0.6.69

* New validated case `age-sway-correlation-bds` — a **cross-sectional cohort** case introducing
  `PhysioAnalysis::correlationTest` and demonstrating the age-related decline in balance. Across 50
  subjects (ages 18-75) of the Santos & Duarte (2016) balance dataset, quiet-standing COP path length
  (from the certified `swayMetrics`) increases with age — on the foam surface Pearson r = 0.62
  (p = 1.5e-6) — and the age-sway correlation is stronger on foam (0.62) than firm (0.39): a compliant
  surface degrades proprioception and unmasks the age-related decline. The correlation reproduces **both**
  `scipy.stats.pearsonr` **and** base R `cor.test` **bit-for-bit**; a chained certification (the inputs
  come from the separately-certified swayMetrics).

# PhysioRecipes 0.6.68

* New validated case `posturography-foam-firm-bds` — a **cohort balance** case introducing
  `PhysioAnalysis::pairedTTest` and demonstrating the sensory-reweighting effect. Across 50 subjects of
  the Santos & Duarte (2016) balance dataset, COP path length (from the certified `swayMetrics`) on a
  foam surface is ~2.76x that on a firm surface (mean 167 vs 61 cm), the paired difference is
  overwhelmingly significant (t = 17.3, df = 49, p ~ 1.6e-22), and foam exceeds firm in **all 50/50
  subjects**. The paired t-test reproduces **both** `scipy.stats.ttest_rel` **and** base R
  `t.test(paired = TRUE)` **bit-for-bit**. Standing on foam degrades ankle proprioception, so the balance
  system reweights toward vision and vestibular input and sways more — a core balance / fall-risk concept,
  and a chained certification (the test's inputs come from the separately-certified swayMetrics).

# PhysioRecipes 0.6.67

* New validated case `sway-posturography-bds` — a **cross-tool audit** certifying the posturography /
  balance metric suite. It validates the shipped `PhysioMoCap::swayMetrics` — not a new op — against an
  independent Python reimplementation of the Prieto et al. (1996) posturography formulas, for the first
  time. On a real 60-s quiet-standing center-of-pressure trial from the Santos & Duarte (2016) public
  balance dataset, the metrics (path length, mean velocity, 95% confidence ellipse/circle area, sway
  area, mean/RMS distance) reproduce the reference **bit-for-bit** (max `|diff|` ~7e-13). Construct check:
  the 95% confidence ellipse contains 96.7% of the COP samples, confirming it is correctly scaled. Sway
  metrics are the standard quiet-stance balance and fall-risk measures in rehabilitation.

# PhysioRecipes 0.6.66

* New validated case `sparc-ldlj-smoothness-wbds` — a **cross-tool audit** certifying two
  neurorehabilitation-standard movement-smoothness metrics. It validates the shipped
  `PhysioMoCap::sparc` (Spectral Arc Length) and `PhysioMoCap::ldlj` (log dimensionless jerk) — not new
  ops — against an independent Python reimplementation of the canonical Balasubramanian et al. (2015)
  smoothness toolbox, for the first time. On a real WBDS gait knee angular-speed cycle both reproduce the
  reference **bit-for-bit** (max `|diff|` ~4e-16 for SPARC, ~1e-14 for LDLJ). A construct check confirms
  SPARC becomes more negative (less smooth) when the movement is corrupted with noise; and across the
  WBDS walking speeds the knee-speed cycle is smoothest at moderate speeds and less smooth at the fastest
  (speed-smoothness correlation −0.26), consistent with movement smoothness peaking near preferred speed.

# PhysioRecipes 0.6.65

* New validated case `ks-normality-eegmmidb` — a **new normality-test op** case introducing
  `PhysioAnalysis::ksNormalityTest` (Kolmogorov-Smirnov), the empirical-CDF counterpart of
  `jarqueBeraTest`. On real eyes-closed EEG its `D` statistic reproduces **both** `scipy.stats.kstest`
  **and** base R `ks.test` **bit-for-bit** (`|diff| ~4e-17` against each). Finding: on the same EEG, KS
  does **not** reject normality (D = 0.026, p = 0.24) even though `jarqueBeraTest` **does** (JB = 15.5,
  p = 4.3e-4) — the two normality tests reach opposite verdicts, because KS reads the overall CDF shape
  while Jarque-Bera flags the mild skew/kurtosis with the high power a large N confers. "Normal" is
  test-dependent. Caveat: the estimated-parameter KS p-value is anti-conservative (Lilliefors); the D
  statistic is exact.

# PhysioRecipes 0.6.64

* New validated case `trimmed-mean-hrv-fantasia` — a **new robust-location op** case introducing
  `PhysioAnalysis::trimmedMean`, applied to HRV, completing the robust toolkit (robust location to go with
  the robust dispersion of `medianAbsDev` / `interquartileRange`). On real Fantasia RR intervals it
  reproduces **both** `scipy.stats.trim_mean` **and** base R `mean(x, trim)` **bit-for-bit** (`|diff| = 0`
  against each) at trim 0.1/0.2/0.25. Finding: on the right-skewed RR the plain mean (767 ms) exceeds the
  10%-trimmed mean (762), which exceeds the 20%-trimmed mean (760), which exceeds the median (756) — the
  trimmed mean interpolates between the mean and the median, and the mean-vs-trimmed gap measures how much
  the heavy RR tail inflates the mean (a robust central heart-period resistant to ectopic beats).
  Complements the `iqr-hrv-fantasia` case (robust dispersion on the same skewed RR).

# PhysioRecipes 0.6.63

* New validated case `iqr-hrv-fantasia` — a **new robust-dispersion op** case introducing
  `PhysioAnalysis::interquartileRange`, applied to HRV. On real Fantasia RR intervals it reproduces
  **both** `scipy.stats.iqr` **and** base R `IQR` **bit-for-bit** (`|diff| = 0` against each). Finding:
  the RR distribution is right-skewed and heavy-tailed (skewness 1.01, excess kurtosis 2.91), so the
  robust IQR-based SD estimate (56.3 ms) sits materially below SDNN (69.3 ms; ratio 0.81) — the opposite
  of the near-Gaussian eyes-closed EEG (`median-abs-dev-eegmmidb`), where the robust scale matched the SD
  to 0.2%. The robust/classical dispersion ratio is thus a distribution-shape diagnostic: here it shows
  SDNN is inflated by the heavy RR tail (respiratory sinus arrhythmia, occasional ectopy) that the robust
  IQR ignores.

# PhysioRecipes 0.6.62

* New validated case `multiscale-entropy-eegmmidb` — a **new standalone op** case introducing
  `PhysioEEG::multiscaleEntropy` (Costa 2002), completing the entropy family. On a real eyes-closed EEG
  channel the per-scale MSE curve reproduces NeuroKit2 `entropy_multiscale` (MSEn) **bit-for-bit** across
  scales 1–8 (max `|diff|` ~4e-16). Finding: the curve is non-monotonic — it rises from scale 1 (1.327)
  to a peak at scale 5 (1.879) and stays above the scale-1 value, whereas a white-noise series of the
  same length and SD falls monotonically (2.16 → 1.08). The rise is the signature of genuine multi-scale
  structure in the eyes-closed EEG (fast activity plus slower alpha-band dynamics). Caveat: coarse-scale
  SampEn is less stable on short coarse-grained series (scale 8 leaves 200 points).

# PhysioRecipes 0.6.61

* New validated case `sampen-apen-eegmmidb` — a **cross-tool audit** of the two most-used regularity
  measures. It certifies the shipped `PhysioMoCap::sampleEntropy` (Richman-Moorman) and
  `approximateEntropy` (Pincus) — not new ops — against the gold-standard Python references on EEG for
  the first time (they were previously certified only against a base-R Pincus recompute on accelerometer
  data). On a real eyes-closed EEG channel both reproduce **both** `antropy` **and** NeuroKit2
  **bit-for-bit** (max `|diff|` ~2e-16 vs each). Finding: sample entropy (1.327) exceeds approximate
  entropy (1.292) — the textbook ApEn self-match bias, in which ApEn counts each template's match with
  itself and so systematically underestimates entropy; SampEn removes the self-count and is the less
  biased estimator.

# PhysioRecipes 0.6.60

* New validated case `median-abs-dev-eegmmidb` — a **new robust-dispersion op** case introducing
  `PhysioAnalysis::medianAbsDev` (the robust companion of `signalMoments`). On a real eyes-closed EEG
  channel the raw MAD reproduces **both** `scipy.stats.median_abs_deviation` **and** base R
  `stats::mad(constant = 1)` **bit-for-bit** (`|diff| = 0` against each): a double-reference agreement,
  because the raw MAD is an unambiguous order statistic. Finding: the normal-consistent MAD-scale (48.93)
  matches the population SD (48.83) to within 0.2% (ratio 1.002) — because MAD is robust and SD is not,
  this agreement is positive evidence the eyes-closed POz distribution is near-Gaussian and artifact-free,
  cross-consistent with the `signalMoments` and Jarque-Bera folds on the same channel. Convention note:
  the raw MAD (constant = 1) is validated exact; the normal-consistent constant `1/qnorm(0.75)` differs
  between base R's rounded 1.4826 and scipy's full-precision value at ~1e-6.

# PhysioRecipes 0.6.59

* New validated case `katz-fd-eegmmidb` — a **new standalone op + cross-tool convention fix** case.
  It introduces `PhysioEEG::katzFD` (Katz's fractal dimension), completing the fractal-dimension family
  (`petrosianFD`, `svdEntropy`). On a real eyes-closed EEG channel it reproduces **two** independent
  references — `antropy.katz_fd` **and** NeuroKit2 `fractal_katz` — **bit-for-bit** (`|diff| = 0` against
  each): a double-reference machine-precision agreement. Integrity finding: the shipped
  `eegComplexity(measure = "katz_fd")` previously used a non-standard Euclidean-plane convention
  (KFD 1.696) that no reference tool produces; this fold unifies it to the field-standard amplitude-only
  convention (KFD 3.468, a ~1.77 change), restoring cross-tool comparability. Honest scope: the
  amplitude-only KFD is not bounded to [1, 2] (real EEG scores above 2, exactly as antropy/NeuroKit2
  report) — a property of the de-facto convention, not an error.

# PhysioRecipes 0.6.58

* New validated case `jarque-bera-eegmmidb` — a **new-method authoring** case adding the formal
  normality test (the confirmatory partner of `signalMoments`, analogous to Ljung-Box for the ACF):
  introduces and certifies a newly-authored op, `PhysioAnalysis::jarqueBeraTest` (Jarque-Bera statistic,
  built on `signalMoments`). On a real eyes-closed EEG channel it reproduces `scipy.stats.jarque_bera`
  **bit-for-bit** for both the statistic and the p-value. Finding (an instructive nuance): **JB=15.5
  rejects normality (p=4.3e-4)** even though `signalMoments` finds the distribution **descriptively
  near-Gaussian** — because at N=1600 the test detects the mild skew (-0.23) and kurtosis (+0.16), the
  same large-N power phenomenon as the decisive ADF/Ljung-Box rejections. The honest lesson: on a long
  signal a normality test almost always rejects, so "approximately Gaussian" is an effect-size statement,
  not a test outcome. Honest scope: newly-authored op; machine-precision (identical closed-form JB +
  chi-square).

# PhysioRecipes 0.6.57

* New validated case `signal-moments-eegmmidb` — a **new-method authoring** case adding the
  amplitude-distribution shape view: introduces and certifies a newly-authored op,
  `PhysioAnalysis::signalMoments` (mean, population SD, skewness, excess kurtosis). On a real eyes-closed
  EEG channel it reproduces `scipy.stats.skew` / `kurtosis` (and numpy mean/std) **bit-for-bit** (max
  |diff| ~**4e-16**). Finding: the amplitude distribution is **approximately Gaussian** (skewness -0.23,
  a mild left asymmetry; excess kurtosis +0.16, near-mesokurtic) — consistent with the normality many
  downstream methods assume, whereas a much larger kurtosis would flag a spiky artifact. Honest scope:
  newly-authored op; machine-precision (identical formulas, biased/population conventions).

# PhysioRecipes 0.6.56

* New validated case `ar-spectrum-eegmmidb` — a **new-method authoring** case delivering the AR model's
  spectral payoff and completing the time-series lane: introduces and certifies a newly-authored op,
  `PhysioAnalysis::arSpectrum` (AR(8) Yule-Walker parametric spectrum), built on `arYuleWalker`. On a
  real eyes-closed EEG channel it reproduces `stats::spec.ar` **bit-for-bit** (max |rel diff| ~**5e-14**,
  including spec.ar's N/(N-p-1) variance convention). Finding: the AR spectral peak (**10.9 Hz**) falls
  in the alpha band and agrees within **~0.5 Hz** with the nonparametric smoothed-periodogram peak
  (11.4 Hz) — the parametric and nonparametric spectral estimates concur on the eyes-closed alpha rhythm,
  each a check on the other. Honest scope: newly-authored op; machine-precision (identical AR spectral
  formula incl. the spec.ar variance convention); a spectral density up to the estimator's scaling.

# PhysioRecipes 0.6.55

* New validated case `kpss-eegmmidb` — a **new-method authoring** case completing the ADF/KPSS
  confirmatory pair: introduces and certifies a newly-authored op, `PhysioAnalysis::kpssTest` (KPSS
  level-stationarity statistic), the complement of `adfTest` (KPSS's null is stationarity, ADF's is a
  unit root). On a real eyes-closed EEG channel its statistic reproduces
  `statsmodels.tsa.stattools.kpss(regression = "c")` **bit-for-bit** (|diff| ~**7e-16**). Finding: the
  KPSS statistic (**0.20**) is below the 5% critical value (0.463), so it **fails to reject
  stationarity**, and this **agrees with the ADF test** (which rejects the unit root) — the two
  complementary tests concur the segment is **stationary**, the strongest form of the conclusion. Honest
  scope: newly-authored op; only the statistic is bit-exact (critical values / p from reference tables);
  agreement is the finding (disagreement would flag long memory / fractional integration).

# PhysioRecipes 0.6.54

* New validated case `adf-eegmmidb` — a **new-method authoring** case adding the stationarity check the
  ACF/PACF/AR/Ljung-Box folds implicitly assumed: introduces and certifies a newly-authored op,
  `PhysioAnalysis::adfTest` (Augmented Dickey-Fuller unit-root statistic). On a real eyes-closed EEG
  channel its statistic reproduces `statsmodels.tsa.stattools.adfuller(autolag = None)` **bit-for-bit**
  (|diff| ~**7e-15**). Finding: the statistic (**-22.36**) lies ~**19.5 below** the 5% Dickey-Fuller
  critical value (-2.86), decisively rejecting the unit-root null — the segment is **stationary** (no
  drift over 10 s), retroactively justifying the companion folds' assumption. Honest scope:
  newly-authored op; only the statistic is bit-exact (an exact OLS quantity) — the critical values and
  p-value require the MacKinnon reference tables and are not computed.

# PhysioRecipes 0.6.53

* New validated case `ljung-box-eegmmidb` — a **new-method authoring** case adding the formal
  counterpart to the ACF/PACF/AR lane: introduces and certifies a newly-authored op,
  `PhysioAnalysis::ljungBoxTest` (Ljung-Box portmanteau white-noise / model-adequacy test, built on
  `autocorrelation`). On a real eyes-closed EEG channel it reproduces the field-standard
  `statsmodels.stats.diagnostic.acorr_ljungbox` **bit-for-bit** for both the Q statistic and the p-value
  (|diff| 0), and matches `stats::Box.test` to ~5e-12. Finding: at lag h=10, **Q=4165 is ~228x the 95%
  chi-square critical value (18.31)**, so the eyes-closed EEG rejects the white-noise null overwhelmingly
  — the formal, quantitative counterpart of the strong ACF/PACF/AR structure of the companion folds.
  Honest scope: newly-authored op; machine-precision (identical closed-form Q + chi-square); on residuals
  the same op tests model adequacy.

# PhysioRecipes 0.6.52

* New validated case `ar-yule-walker-eegmmidb` — a **new-method authoring** case completing the
  ACF/PACF/AR-fit lane: introduces and certifies a newly-authored op, `PhysioAnalysis::arYuleWalker`
  (AR(p) fit by Yule-Walker via Durbin-Levinson on the biased ACF, building on `autocorrelation` /
  `partialAutocorrelation`). On a real eyes-closed EEG channel it reproduces the field-standard
  `statsmodels.regression.yule_walker(method = "mle")` **bit-for-bit** for both the AR coefficients (max
  |diff| ~**5e-15**) and the innovation variance (~**5e-13**). Finding: the fitted AR(4) is **stationary**
  (all roots outside the unit circle, min |root| ~1.09) with a negative lag-2 coefficient, and its
  parametric spectrum peaks in the **alpha band (~12 Hz)** — a valid oscillatory generative model of
  eyes-closed alpha. Honest scope: newly-authored op (MLE/biased convention; adjusted and base-R ar.yw
  correction differ by construction); machine-precision (identical Yule-Walker).
